        <!--====== Start Hero Section ======-->
        <section class="hero-area">
            <div class="hero-wrapper-one">
                <div class="container">
                    <div class="row">
                        <div class="col-lg-7">
                            <div class="hero-content">
                                <h1 class="wow fadeInUp" wow-data-delay="30mss" style="font-size: 5rem;font-weight: bold">Beragam Kode Voucher dan Penawaran Disini</h1>
                                <!-- <h3 class="wow fadeInDown" wow-data-delay="50ms">People Don’t Take,Trips Take People</h3> -->
                                <div class="hero-search-wrapper wow fadeInUp" wow-data-delay="70ms">
                                    <form action="/dashboard">
                                        <div class="row">
                                            <div class="col-lg-9 col-md-5 col-sm-12">
                                                <div class="form_group">
                                                    <input type="search" class="form_control" placeholder="Cari Berdasarkan Kategori" name="search" value="{{ request('search') }}">
                                                    <i class="ti-ink-pen"></i>
                                                </div>
                                            </div>

                                            <div class="col-lg-3 col-md-4 col-sm-12">
                                                <div class="form_group">
                                                    <button type="submit" class="main-btn icon-btn">Cari</button>
                                                </div>
                                            </div>
                                        </div>
                                    </form>
                                </div>
                                <p class="tags"><span>Temukan:</span>Kode Voucher dan Kode Promo Terbaru</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!--====== End Hero Section ======-->
